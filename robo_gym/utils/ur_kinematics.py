...
class kinematics_model():
    ...
    """
    def __init__(self, ur_model='ur5', gripper_offset=0.13, origin_offset=None):

        #DH params
        #https://www.universal-robots.com/articles/ur/application-installation/dh-parameters-for-calculations-of-kinematics-and-dynamics/

        # the robots origin: where is it placed? It is 0.1m above the ground in ur_control.launch.xml
        ...
        #distance (m)
        if ur_model == 'ur3':
            self.d=np.array([0.1519, 0, 0, 0.11235, 0.08535, 0.0819 + gripper_offset], dtype=np.float32)
            self.a=np.array([0, -0.24365, -0.21325, 0, 0, 0], dtype=np.float32)
            ...
            self.origin=np.array(origin_offset, dtype=np.float32)
            self.arm_A_radius = 0.066 / 2

        elif ur_model == 'ur3e':
            self.d=np.array([0.15185, 0, 0, 0.13105, 0.08535, 0.0921 + gripper_offset], dtype=np.float32)
            self.a=np.array([0, -0.24355, -0.2132, 0, 0, 0], dtype=np.float32)
            ...
            self.d = np.array([0.1625, 0, 0, 0.13333, 0.0997, 0.0996 + gripper_offset], dtype=np.float32)
            self.a = np.array([0, -0.425, -0.3922, 0, 0, 0], dtype=np.float32)
            ...
            self.d = np.array([0.1807, 0, 0, 0.17415, 0.11985, 0.11655 + gripper_offset], dtype=np.float32)
            self.a = np.array([0, -0.6127, -0.57155, 0, 0, 0], dtype=np.float32)
            ...
            self.d = np.array([0.1807, 0, 0, 0.17415, 0.11985, 0.11655 + gripper_offset], dtype=np.float32)
            self.a = np.array([0, -0.4784, -0.36, 0, 0, 0], dtype=np.float32)
        ...
        for joint_combination in possible_joints:
            ...
            for joint_index in range(len(joints_without_offset)):
                ...
                if(frame_center_z_coord<0 and joint_index!=1) or (frame_center_z_coord<(4*self.arm_A_radius/3) and joint_index==1):
                    break
                # all joints from the combination are valid, append to valid joints
